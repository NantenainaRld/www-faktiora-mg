...
class AutreEntree extends Database
{
    ...
    //static - restore all autre entree
    public static function restoreAllAutreEntree($nums_ae)
    {
        $response = ['message_type' => 'success', 'message' => 'success'];

        $placeholders = implode(', ', array_fill(0, count($nums_ae), '?'));
        $sql = "UPDATE autre_entree SET etat_ae = 'actif' WHERE num_ae IN ({$placeholders}) AND etat_ae = 'supprimé'";
        ...
        try {
            ...
            //0
            if ($response['row_count'] === 0) {
                $response['message'] = __('messages.success.entree_restoreAllAutreEntree_0');
            }
            //1
            elseif ($response['row_count'] === 1) {
                $response['message'] = __('messages.success.entree_restoreAllAutreEntree_1');
            }
            //plur
            else {
                $response['message'] = __('messages.success.entree_restoreAllAutreEntree_plur', ['field' => $response['row_count']]);
            }
